...
void fnSQRT()
{
  ...
  else { outputSTR = "Square Root: positive number."; }
  //functionType = 0;
}
...
void fnINVERSE()
{
  ...
  else { outputSTR = "Inverse: non-zero number."; }
}
...
void fnCBRT()
{
  ...
  else { outputSTR = "Cube Root: positive number."; }
}
...
void fnLN()
{
  ...
  else { outputSTR = "LN: positive number."; }
  functionType = 0;
}
...
void fnLOG()
{
  ...
  else { outputSTR = "LOG10: positive number."; }
  functionType = 0;
}
...
void fnASIN()
{
  ...
  else { outputSTR = "ArcSin: number in [-1, 1]."; }
  functionType = 0;
}
...
void fnACOS()
{
  ...
  else { outputSTR = "ArcCos: number in [-1, 1]."; }
  functionType = 0;
}
...
void fnACOSH()
{
  ...
  else { outputSTR = "ArcCosh: number >= 1."; }
  functionType = 0;
}
...
void fnDD() // from degrees minute second to decimal degrees
{
  ...
  else { outputSTR = "Decimal Degrees: three numbers (degree min sec)."; }
}
//------------------------------------------------------------
...
void fnROUND()
{
  if (pila.stackSize() > 0)
  {
    ...
    else { outputSTR = "Round: integer [0, 9]"; }
  }
  else { outputSTR = "Round: two numbers required."; }
  functionType = 0;
}
...
void fnPOLAR()
{
  ...
  else { outputSTR = "To Polar: two numbers required."; }
  functionType = 0;
}
...
void fnRECT()
{
  ...
  else { outputSTR = "To Cartesian: two numbers required."; }
  functionType = 0;
}
...
void fnFACT()
{
  ...
  else { outputSTR = "Factorial: positive integer number."; }
  functionType = 0;
}
...
void fnFIBO()
{
  ...
  else { outputSTR = "Fibonacci: positive integer number."; }
  functionType = 0;
}
// Fibonacci
// F(n+2) = F(n+1) + F(n)
...
void fnHORNER()
{
  if (pila.stackSize() > 2)
  {
    ...
    else
    {
      pila.push(grado);
      digitNUM = x;
      outputSTR = "Polynomial: bad number of coefficients or degree.";
      functionType = 0;
      return;
    }
...
void fnCUBIC()
{
  if (pila.stackSize() > 2)
  {
    ...
    if ( a == 0.0 ) // Quadratic Equation --> Use Solve Quaddratic
    {
      ...
      isResult = true;
      isBlocked = false;
      outputSTR = "Cubic: a*x^3 = 0 --> use Quadratic.";
      functionType = 0;
      return;
    }
    // solve cubic equation
  ...
  else { outputSTR = "Cubic: four numbers required."; }
  functionType = 0;
}
...
void fnTOHSB()
{
  if (pila.stackSize() > 1)
  {
       ...
       (rr == r0 && gg == g0 && bb == b0))
    {
      ...
      // create the color
      color crgb = color(rr, gg, bb);
      // extract HSB values while in RGB colorMode
      // note that hue precision involves rounding
      float hh = hue(crgb);
      float ss = saturation(crgb);
      float vv = brightness(crgb);
      hh = map(hh,0.0,255.0,0.0,360.0);
      ss = map(ss,0.0,255.0,0.0,100.0);
      vv = map(vv,0.0,255.0,0.0,100.0);
      pila.push(vv);
      pila.push(ss);
      outputSTR = String.valueOf(hh);
      isResult = true;
      isBlocked = false;
    }
    else
    {
      ...
      pila.push(g0);
      digitNUM = b0;
      outputSTR = "To HSB: RGB integer values range [0,255].";
    }
  }
  else
  {
    outputSTR = "To HSB: three numbers required.";
  }
  functionType = 0;
}
...
void fnTORGB()
{
  if (pila.stackSize() > 1)
  {
    ...
    float vv = v0;
    println(hh,ss,vv);
    if (hh >= 0 && hh < 360 && ss >= 0 && ss <= 100 && vv >= 0 && vv <= 100)
    //&& (hh == h0 && ss == s0 && vv == v0))
    {
      lastX = digitNUM;
      hh = map(hh, 0.0,360.0, 0.0,255.0);
      ss = map(ss, 0.0,100.0, 0.0,255.0);
      vv = map(vv, 0.0,100.0, 0.0,255.0);
      // HSB color mode
      colorMode(HSB, 255.0, 255.0, 255.0);
      // create the color with HSB values
      // note that this is stored as RGB
      color chsb = color(hh, ss, vv);
      // extract RGB values while in HSB colorMode
      float rr = red(chsb);
      float gg = green(chsb);
      float bb = blue(chsb);
      println(rr,gg,bb);
    ...
    else
    {
      ...
      pila.push(s0);
      digitNUM = v0;
      outputSTR = "To RGB: HSB range [0,360) [0,100] [0,100].";
    }
  }
  else
  {
    outputSTR = "To RGB: three numbers required.";
  }
  functionType = 0;
}